export class Shop {
  constructor(scene) {
    ...
    this._buyBtn2    = scene.add.text(PX + PW - 20, PY + 196, '[ Comprar ]',
                         { ...ts, fontSize: '13px', color: '#88ff88' })
                         .setOrigin(1, 0).setInteractive({ useHandCursor: true }).setDepth(21).setScrollFactor(0);
  ...
  update(delta) {
    ...
    if (this._player) {
      ...
      // Atualiza estado do botão de radar se já tiver
      const hasR = this._player.hasRadar;
      this._buyBtn2.setText(hasR ? '[ Obtido ]' : '[ Comprar ]').setColor(hasR ? '#888888' : '#88ff88');
    }
